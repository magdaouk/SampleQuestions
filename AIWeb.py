...
def get_recommendation(recommendation_type, first_name, last_name, company, title, years_worked, skills_emphasis, relationship, user_first_name, user_last_name, user_company, user_title, manual_notes, pronoun=None):
    skills = {
    ...
    skills_text = "\n\n".join([f"- {skill}: {emphasis}" for skill, emphasis in skills.items() if emphasis == 1])
    ...
    if recommendation_type == "Letter of Recommendation":
        context += f"Write a formal Letter of Recommendation without a header and signature for a job addressed to the hiring manager. The recommendation is for a candidate named {first_name} {last_name}, who is a {title} at {company} and has worked together for {years_worked} years in the capacity of {relationship}. Emphasize the following skills:\n\n{skills_text}"
    else:
